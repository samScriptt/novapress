...
export default async function Home({ searchParams }: HomeProps) {
  ...
  const itemsFirstPage = 9;  // 1 Hero + 8 Grid
  const itemsOtherPages = 12; // 12 Grid
  ...
  if (currentPage === 1) {
    ...
    from = itemsFirstPage + (currentPage - 2) * itemsOtherPages;
    to = from + itemsOtherPages - 1;
  }
  // ------------------------------------
  ...
  let totalPages = 1;
  if (count) {
    if (count <= itemsFirstPage) {
        totalPages = 1;
    } else {
        totalPages = 1 + Math.ceil((count - itemsFirstPage) / itemsOtherPages);
    }
  }
