      ...
      <div className="container mx-auto px-4 h-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-2 group text-black dark:text-green-500">
                ...
                <div className="font-mono font-bold tracking-tighter text-lg uppercase min-w-[100px]">
                    {/* Aqui está o Typewriter escrevendo "NovaPress" limpo */}
                    NovaPress
                </div>
            </Link>
        ...
        <nav className="hidden md:flex gap-6 text-xs font-bold uppercase font-mono text-zinc-500 dark:text-green-800">
            {['Tech', 'World', 'AI', 'Economy', 'Science'].map((item) => (
                <Link key={item} href={`/category/${item.toLowerCase()}`} className="hover:text-black dark:hover:text-green-400 transition-colors">
                    {item}
                </Link>
            ))}
        </nav>
        ...
        <div className="flex items-center gap-3">
            <div className="scale-90"><SearchBar /></div>
            <div className="scale-90"><ThemeToggle /></div>
            
            {user ? (
                <div className="relative">
                    <button onClick={() => setMenuOpen(!menuOpen)} className="w-8 h-8 bg-zinc-100 dark:bg-green-900/20 border border-zinc-300 dark:border-green-700 text-black dark:text-green-400 flex items-center justify-center text-xs font-bold font-mono hover:bg-zinc-200 dark:hover:bg-green-900/40 transition">
                    ...
                    {menuOpen && (
                        <div className="absolute right-0 top-10 w-48 bg-white dark:bg-black border border-zinc-200 dark:border-green-800 p-1 shadow-xl font-mono z-50">
                            ...
                            <button onClick={() => logout()} className="flex items-center gap-2 px-3 py-2 text-xs w-full text-left hover:bg-zinc-100 dark:hover:bg-green-900/30 dark:text-green-400 text-red-600">
                ...
                <Link href="/login" className="text-zinc-500 dark:text-green-600 hover:text-black dark:hover:text-green-400 transition"><User size={18} /></Link>
            )}
        </div>
      </div>
    </header>
